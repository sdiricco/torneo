<template>
  <div class="surface-ground text-color">
    ...
    <template v-else>
      ...
      <FirstTeams :teams="getTeamsRanking" :max-teams-visible="3" class="m-4" @goToRankingPage="emit('goToRankingPage')" />
      <div class="text-xl font-bold px-4 pt-4 flex align-items-center">
        <font-awesome-icon icon="fa-solid fa-people-group" class="mr-4"></font-awesome-icon>
        <div>Classifica marcatori</div>
      </div>
      <FirstPlayers :players="playersStats" :max-players-visible="3" class="m-4" @go-to-ranking-page="emit('goToPlayersPage')" />
